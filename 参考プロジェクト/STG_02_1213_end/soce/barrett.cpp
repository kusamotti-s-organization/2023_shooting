...
void BarrettUpdate() {
	for (int i = 0; i < barrettnums; i++) {
		...
		for (int j = maxlog-1; j >0 ; j--) {
			...
			barrett[i].logs[j].x=barrett[i].logs[j-1].x;
			barrett[i].logs[j].y=barrett[i].logs[j-1].y;
		}
...
void BarrettDraw() {
	for (int i = 0; i < barrettnums; i++) {
		if (barrett[i].active) {
			...
			//DrawCircle(barrett[i].pos.x,barrett[i].pos.y,barrett[i].size/2,GetColor(barrett[i].color.r, barrett[i].color.g, barrett[i].color.b),false);
			//DrawGraph(barrett[i].pos.x-barrett[i].size/2,barrett[i].pos.y -barrett[i].size/2,barrett[i].img,true);
